...
[{ id: "inmueble", estado: "on", titulo: "casa grande", descripcion: "0", ciudad: "", precioPorNoche: 150, promedioCalif: 500, usuarioAnfitrion: "Camiiii_ok" , fotos : [0,1,2]},
    ...
    { id: "inmueble", estado: "on", titulo: "playa grande", descripcion: "2", ciudad: "", precioPorNoche: 350, promedioCalif: 350, usuarioAnfitrion: "Camiiii_ok" , fotos : [6,7,8]},
    { id: "inmueble", estado: "on", titulo: "casita", descripcion: "3", ciudad: "", precioPorNoche: 450, promedioCalif: 250, usuarioAnfitrion: "Camiiii_ok" , fotos : [9,10,11]},
    { id: "inmueble", estado: "on", titulo: "casota", descripcion: "4", ciudad: "", precioPorNoche: 550, promedioCalif: 100, usuarioAnfitrion: "Robert_63" , fotos : [12,13,14]},
    { id: "inmueble", estado: "on", titulo: "casona", descripcion: "5", ciudad: "", precioPorNoche: 650, promedioCalif: 90, usuarioAnfitrion: "Robert_63" , fotos : [15,16,17]},
    { id: "inmueble", estado: "on", titulo: "swite mediana", descripcion: "6", ciudad: "", precioPorNoche: 750, promedioCalif: 850, usuarioAnfitrion: "Pepe_1" , fotos : [18,19,20]},
    { id: "inmueble", estado: "on", titulo: "chalet grande", descripcion: "7", ciudad: "", precioPorNoche: 50, promedioCalif: 350, usuarioAnfitrion: "Pepe_1" , fotos : [21,22,23]},
    { id: "inmueble", estado: "on", titulo: "casa mediana", descripcion: "8", ciudad: "", precioPorNoche: 10, promedioCalif: 850, usuarioAnfitrion: "Pepe_1" , fotos : [26,24,25]},
    { id: "inmueble", estado: "on", titulo: "casita chica", descripcion: "9", ciudad: "", precioPorNoche: 20, promedioCalif: 450, usuarioAnfitrion: "Pepe_1" , fotos : [29,27,28]},
    { id: "inmueble", estado: "on", titulo: "casita 10", descripcion: "10", ciudad: "", precioPorNoche: 30, promedioCalif: 250, usuarioAnfitrion: "Homer.78" , fotos : [30,31,32]},
    { id: "inmueble", estado: "on", titulo: "casa 11", descripcion: "11", ciudad: "", precioPorNoche: 40, promedioCalif: 350, usuarioAnfitrion: "Homer.78" , fotos : [33,35,34]},
    { id: "inmueble", estado: "on", titulo: "casa 13", descripcion: "12", ciudad: "", precioPorNoche: 50, promedioCalif: 150, usuarioAnfitrion: "Robert_63" , fotos : [36,37,38]},
    { id: "inmueble", estado: "on", titulo: "casa 14", descripcion: "13", ciudad: "", precioPorNoche: 60, promedioCalif: 2050, usuarioAnfitrion: "Homer.78" , fotos : [39,40,1]},
    { id: "inmueble", estado: "on", titulo: "casa 15", descripcion: "14", ciudad: "", precioPorNoche: 70, promedioCalif: 150, usuarioAnfitrion: "Homer.78" , fotos : [2,3,4]},
...
if (userActual === "Huesped") {
    if(document.getElementById("divCotizacion").children[0].value === "U$D")
    {
       ...
       <tr><td>Ciudad: ${element.ciudad}</td></tr>
       <tr><td>Precio por noche: <span class="precio">${cotizacion*Number(element.precioPorNoche)}</span></td></tr>
       <tr><td>Calificacion: ${element.promedioCalif}</td></tr><div id="${element.id}"></div>
       </table><br><br>`;}
   });
   }else{listaInmuebles.forEach(element => {
    if(element.estado === "on"){
...
/*function buscador() {
    ...
    let arrayBuscado = new Array();
    let tituloEncontrado = false;
    let descripcionEncontrado = false;
    listaInmuebles.forEach(element => {
        if(element.titulo === txtBuscado)
        {
            arrayBuscado.push(element);
    ...
    if(tituloEncontrado || descripcionEncontrado){
        return arrayBuscado;
    }
}
...
function buscador() {
    ...
    if (!verificarTextoNoVacio(txtBuscadoPro)) {
        let txtBuscadoPalabras = devuelveArray(txtBuscadoPro);
        
        if (user !== "Anfitrion") {
            listaInmuebles.forEach(element => {

                let tituloPro = txtNoTildesNiMayus(element.titulo);
                let tituloProPalabras = devuelveArray(tituloPro);
                let ciudadPro = txtNoTildesNiMayus(element.ciudad);
                let ciudadProPalabras = devuelveArray(ciudadPro);
                let encontre = false;

                txtBuscadoPalabras.forEach(eleB => {

                    for (let i = 0; i < tituloProPalabras.length && !encontre; i++) {

                        let palabraBuscada = tituloProPalabras[i];

                        if (eleB == palabraBuscada) {
                            encontre = true;
                            arrayResultado.push(eleA);
                        }
                        if (!encontre) {
                            for (let ite = 0; ite < ciudadProPalabras.length && !encontre; ite++) {
                                palabraBuscada = ciudadProPalabras[ite];
            ...
            if (user == "Anfitrion") {
                listaInmuebles.forEach(eleA => {

                    let descripPro = txtNoTildesNiMayus(element.descripcion);
                    let descripProPalabras = devuelveArray(descripPro);

                    descripProPalabras.forEach(eleB => {

                        for (let i = 0; i < descripProPalabras.length && !encontre; i++) {
                            let palabraBuscada = txtBuscadoPalabras[i];
                            let usuInmueble = element.usuarioAnfitrion;

                            if (usuInmueble === user && eleB == palabraBuscada) {
                                encontre = true;
                                arrayResultado.push(eleA);
                            }
                        }
...
function devuelveArray(txt) {

    let arrayPalabras = new Array();
    let palabra = "";

    for (let i = 0; i < txt.length; i++) {
        let letraActual = txt.charAt(i);
        let esta = esEspacio(letraActual);
...
function txtNoTildesNiMayus(txt) {
    ...
    console.log(`Resultado txtNoTildesNiMayus: ${txtSintildesNiMayus}`)
